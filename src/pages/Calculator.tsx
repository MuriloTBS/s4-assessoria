...
export default function Calculator() {
  const { user } = useAuth()
  const params = paramsApi.get(user!.id)

  const [hourlyRate, setHourlyRate]     = useState(params.hourly_rate)
  const [hours, setHours]               = useState(10)
  const [complexity, setComplexity]     = useState<Complexity>(params.default_complexity)
  const [extras, setExtras]             = useState(0)
  const [margin, setMargin]             = useState(params.default_margin)
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          <Card color="blue" className="p-5">
            ...
            <p className="text-3xl font-bold text-white">{formatCurrency(result.withMargin)}</p>
          </Card>
          ...
          <Card className="p-5 space-y-3">
            ...
            {[
              ...
              <div key={label} className="flex justify-between items-center text-sm">
                <span className="text-[#8a9bb0]">{label}</span>
                <span className="text-[#e2e8f0] font-medium">{formatCurrency(value)}</span>
              </div>
            ))}
          </Card>
          ...
          <Card color="green" className="p-4">
            <p className="text-green-400 text-xs font-medium">
              Fator de complexidade: <strong className="text-white">×{COMPLEXITY_FACTOR[complexity]}</strong> ({complexity})
            </p>
          </Card>
        </div>
      </div>
