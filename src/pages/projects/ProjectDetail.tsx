...
import { projectApi, stepApi } from '@/lib/storage'
...
export default function ProjectDetail() {
  ...
  const project = projectApi.get(Number(id))

  const rerender = useCallback(() => setRefresh(r => r + 1), [])
  ...
  function addStep() {
    ...
    stepApi.create({ project_id: project!.id, title: newStep.trim(), completed: false, position: steps.length })
    setNewStep('')
    rerender()
  }

  function toggleStep(s: ProjectStep) {
    stepApi.update(s.id, { completed: !s.completed })
    rerender()
  }
  ...
  function saveEditStep() {
    ...
    stepApi.update(editingStep.id, { title: editingStep.title })
    setEditingStep(null)
    rerender()
  }

  return (
    <div className="p-6 space-y-6 max-w-4xl" key={refresh}>
      {/* Header */}
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium ${statusColor(project.status)}`}>
              {project.status}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-4">
          ...
          <p className="text-white font-semibold">{project.value ? formatCurrency(project.value) : '—'}</p>
        </Card>
        <Card className="p-4">
          <p className="text-[#8a9bb0] text-xs uppercase tracking-wide mb-1">Prazo</p>
          <p className="text-white font-semibold">{project.deadline ? formatDate(project.deadline) : '—'}</p>
        </Card>
        <Card className="p-4">
          <p className="text-[#8a9bb0] text-xs uppercase tracking-wide mb-1">Progresso</p>
          <p className="text-white font-semibold">{pct}% <span className="text-[#8a9bb0] font-normal text-xs">({done}/{steps.length} etapas)</span></p>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card className="p-5">
              ...
              <div key={s.id} className="flex items-center gap-2 group">
                ...
                {editingStep?.id === s.id ? (
                  ...
                  <span
                    ...
                    className={`flex-1 text-sm ${s.completed ? 'line-through text-[#8a9bb0]' : 'text-[#e2e8f0]'}`}
                  >
                    {s.title}
                  </span>
                )}
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[#0D1B2A] border border-[#2a3f5f] rounded-xl px-3 py-2 text-sm text-[#e2e8f0] placeholder-[#8a9bb0] focus:outline-none focus:border-blue-500 transition-all"
            />
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            <p className="text-[#8a9bb0] text-sm whitespace-pre-wrap">{project.notes || 'Nenhuma observação.'}</p>
          </Card>
          <Card className="p-5">
          ...
          <Card className="p-5">
            ...
            <p className="text-[#8a9bb0] text-sm whitespace-pre-wrap">{project.description || 'Sem descrição.'}</p>
          </Card>
        </div>
      </div>
    </div>
